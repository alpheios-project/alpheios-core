...
class AlpheiosTreebankAdapter extends _clAdapters_adapters_base_adapter__WEBPACK_IMPORTED_MODULE_1__.default {
  ...
  */
  transform (jsonObj, targetWord, config) {
    ...
    const lang = new alpheios_data_models__WEBPACK_IMPORTED_MODULE_0__.Language(model.languageCode)
    let lemma = new alpheios_data_models__WEBPACK_IMPORTED_MODULE_0__.Lemma(lemmaText, lang) // eslint-disable-line prefer-const
...
class AlpheiosChineseLocAdapter extends _clAdapters_adapters_base_adapter__WEBPACK_IMPORTED_MODULE_0__.default {
  ...
  get language () { return alpheios_data_models__WEBPACK_IMPORTED_MODULE_1__.ChineseLanguageModel.language }
  ...
  _transformData (cedictEntries, targetWord) {
    ...
    wordEntries.forEach(entry => {
      ...
      let lemma = new alpheios_data_models__WEBPACK_IMPORTED_MODULE_1__.Lemma(headword, this.language, []) // eslint-disable-line prefer-const
      ...
      const shortDefs = entry.definitions.map(
        entry => new alpheios_data_models__WEBPACK_IMPORTED_MODULE_1__.Definition(entry, alpheios_data_models__WEBPACK_IMPORTED_MODULE_1__.Language.ENGLISH, alpheios_data_models__WEBPACK_IMPORTED_MODULE_1__.Constants.MIMETypes.TEXT_PLAIN, headword)
      )
      ...
      if (deftexts) {
        for (const d of deftexts) {
          ...
          try {
            ...
            const lang = new alpheios_data_models__WEBPACK_IMPORTED_MODULE_0__.Language(config.langs.target, { normalize: true })
            const def = new alpheios_data_models__WEBPACK_IMPORTED_MODULE_0__.Definition(text, lang, format, lexeme.lemma.word)
      ...
      fullDefDataRes.then(
        async (fullDefData) => {
          if (fullDefData && fullDefData.match(/alph:error|alpheios-lex-error/)) {
            ...
            const lang = new alpheios_data_models__WEBPACK_IMPORTED_MODULE_0__.Language(config.langs.target, { normalize: true })
            const def = new alpheios_data_models__WEBPACK_IMPORTED_MODULE_0__.Definition(fullDefData, lang, alpheios_data_models__WEBPACK_IMPORTED_MODULE_0__.Constants.MIMETypes.TEXT_PLAIN, request.lexeme.lemma.word)
            const definition = alpheios_data_models__WEBPACK_IMPORTED_MODULE_0__.ResourceProvider.getProxy(provider, def)
...
data.setLemmaParser(function (lemma) {
  ...
  if (primary) {
    const lang = new alpheios_data_models__WEBPACK_IMPORTED_MODULE_1__.Language(this.model.languageCode)
    parsed = new alpheios_data_models__WEBPACK_IMPORTED_MODULE_1__.Lemma(primary, lang, parts)
  }
  ...
  */
  static init () {
    if (cachedConfig.size === 0) {
      for (const category in _clAdapters_adapters_adapters_config_json__WEBPACK_IMPORTED_MODULE_11__) {
        let adapters = {} // eslint-disable-line prefer-const
        for (const adapterKey in _clAdapters_adapters_adapters_config_json__WEBPACK_IMPORTED_MODULE_11__[category]) {
          const adapterData = _clAdapters_adapters_adapters_config_json__WEBPACK_IMPORTED_MODULE_11__[category][adapterKey]
  ...
  */
  static checkMethod (category, adapterName, methodName) {
    if (!cachedConfig.get(category)[adapterName].methods.includes(methodName)) {
      throw new _clAdapters_errors_wrong_method_error__WEBPACK_IMPORTED_MODULE_9__.default(category, adapterName, methodName)
    }
  }
...
class ImportMorphData {
  ...
  constructor (model, engine) {
    ...
    this.parseLemma = function (lemma) { return new alpheios_data_models__WEBPACK_IMPORTED_MODULE_0__.Lemma(lemma, this.model.language) }

    // may be overridden by specific engine use via setMeaningParser
    this.parseMeaning = function (meaning, targetWord) {
      const lang = meaning.lang ? new alpheios_data_models__WEBPACK_IMPORTED_MODULE_0__.Language(meaning.lang) : alpheios_data_models__WEBPACK_IMPORTED_MODULE_0__.Language.ENGLISH
      return new alpheios_data_models__WEBPACK_IMPORTED_MODULE_0__.Definition(meaning.$, lang, alpheios_data_models__WEBPACK_IMPORTED_MODULE_0__.Constants.MIMETypes.TEXT_PLAIN, targetWord)
    }
