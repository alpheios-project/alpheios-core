...
export default class Assertion {
  /**
   * @param {User} author - A user who made an assertion.
   * @param {object} options - Optional parameters.
   * @param {number} [options.confidence] - A level of confidence, a whole number.
   * @param {Date} [options.dateTime] - When an assertion was made.
   */
  constructor (author, { confidence = 1, dateTime = new Date() } = {}) {
    if (!confidence) { throw new Error(Assertion.errMsgs.CONFIDENCE_IS_MISSING) }
    if (!author) { throw new Error(Assertion.errMsgs.AUTHOR_IS_MISSING) }
    if (!(author instanceof User)) { throw new Error(Assertion.errMsgs.AUTHOR_TYPE_MISMATCH) }
    if (!dateTime) { throw new Error(Assertion.errMsgs.DATETIME_IS_MISSING) }
    if (!(dateTime instanceof Date)) { throw new Error(Assertion.errMsgs.DATETIME_TYPE_MISMATCH) }
