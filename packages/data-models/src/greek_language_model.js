...
class GreekLanguageModel extends LanguageModel {
  ...
  constructor () {
    super()
    this.sourceLanguage = LanguageModel.LANG_GREEK
    this.contextForward = 0
    this.contextBackward = 0
    this.direction = LanguageModel.DIR_LTR
    this.baseUnit = LanguageModel.UNIT_WORD
    this.languageCodes = GreekLanguageModel.languageCodes;
    this.defaultLanguageCode = GreekLanguageModel.languageCodes[0]
  ...
  static supportsLanguage (code) {
    return GreekLanguageModel.languageCodes.includes(code)
  }
  ...
  _initializeFeatures () {
    ...
    features[Feature.types.number] = new FeatureType(Feature.types.number, ['singular', 'dual', 'plural'], langCode)
    features[Feature.types.grmCase] = new FeatureType(Feature.types.grmCase, ['nominative', 'genitive', 'dative', 'accusative', 'vocative'], langCode)
    features[Feature.types.declension] = new FeatureType(Feature.types.declension, ['first', 'second', 'third'], langCode)
    features[Feature.types.gender] = new FeatureType(Feature.types.gender, ['masculine', 'feminine'], langCode)
