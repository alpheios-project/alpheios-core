  ...
  doSpaceSeparatedWordSelection () {
    ...
    */
    ...
    if (this.language.contextForward || this.language.contextBackward) {
      ...
      // pre_wordlist array
      if (pre_wordlist.length > this.language.contextBackward + 1) {
        pre_wordlist = pre_wordlist.slice(pre_wordlist.length - (this.language.contextBackward + 1))
      }
      // limit to the requested # of context words
      // following to the selected word
      if (post_wordlist.length > this.language.contextForward) {
        post_wordlist = post_wordlist.slice(0, this.language.contextForward)
      }
